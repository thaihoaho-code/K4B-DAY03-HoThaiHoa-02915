...
class MCPSupplyChainServer:
    ...
    def call_tool(self, tool_name: str, arguments: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # --------------------------------------------------------------------------
        result = dispatch_tool_call(tool_name, arguments)  # Gọi hàm dispatch_tool_call để lấy chuỗi JSON kết quả từ Tool Router
        content = json.loads(result)  # Chuyển đổi chuỗi JSON kết quả thành Python Dictionary
        return {
            "jsonrpc": "2.0",
...
if __name__ == "__main__":
    ...
    status_tool = next((t for t in tools if t.get("name") == "update_order_status"), None)
    if status_tool and not status_tool.get("parameters", {}).get("properties"):
    ...
    test_result = server.call_tool("order_query", {"order_id": "DH2026002"})
    if not test_result:
        print("⏳ [TODO 2.1]: Hàm call_tool() đang trả về rỗng. Học viên hãy hoàn thiện TODO 2.1 trong 'src/mcp_server.py'!")
    else:
        print(f"✅ [TODO 2.1]: Test dispatch tool 'order_query' thành công:")
